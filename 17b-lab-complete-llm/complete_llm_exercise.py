...
def decode_step_with_cache(tok_id, params, cache):
    ...
    """
    # 1. Embed single token: [1, d_model]
    x0 = [params["E"][tok_id][:]]
    ...
    x1 = [x0[0][j] + attn_out[j] for j in range(d_model)]
    ...
    H_swiglu = [[silu(H_gate[0][j]) * H_up[0][j] for j in range(d_ffn)]]
    FFN_out = matmul(H_swiglu, params["W_down"])[0]

    # 10. Add residual highway: x2 = x1 + ffn_out
    x2 = [x1[j] + FFN_out[j] for j in range(d_model)]
