...
## 步骤 1：3 岁孩子也能懂的直觉（透明描摹纸与神奇的特快高架桥）

> [!INTUITION] 透明描摹纸与神奇的特快高架桥
...
何恺明等人在 2015 年提出的深度残差学习（<dfn id="def-resnet-zh">ResNet</dfn>）中，打破了“让网络直接拟合目标映射 $\mathcal{H}(\mathbf{x})$”的传统观念，转而让网络层去学习差量映射：$\mathcal{F}(\mathbf{x}) = \mathcal{H}(\mathbf{x}) - \mathbf{x}$。
...
<details>
<summary><strong>单位矩阵 $\mathbf{I}$ 如何彻底终结梯度消失？</strong></summary>
仔细观察这一代数形式所蕴含的优雅美感：
...
<figcaption><strong>图 12.2：</strong> Post-LN 将归一化放在主干上阻塞了梯度通道；现代 Pre-LN 架构则将归一化移入支路，保证主干恒等通路的绝对纯净。</figcaption>
</figure>
...
<figcaption><strong>图 12.3：</strong> 从循环神经网络加法记忆到现代大模型 Pre-LN 残差通道的发展历程。</figcaption>
</figure>
...
<fieldset>
<legend><strong>网络层连接范式对比</strong></legend>

<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <caption><strong>表 12.1：</strong> 各种短路连接方案的核心特性对比。</caption>
  <thead>
    <tr bgcolor="#f0eee6">
      <th align="left">架构设计</th>
      <th align="center">前向传播数学形式</th>
      <th align="center">新增参数</th>
      <th align="left">反向梯度传播表现</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><strong>传统无短路网络</strong></td>
      <td align="center">$\mathbf{x}_{l} = \mathcal{F}(\mathbf{x}_{l-1})$</td>
      <td align="center">$0$</td>
      <td>
        <del>面临梯度消失与爆炸崩溃。</del> 梯度随雅可比矩阵连乘 $\prod \mathbf{J}_l$ 指数级衰减，网络通常无法突破 20 层。
      </td>
    </tr>
    <tr>
      <td><strong>高速公路网络 (Highway Net)</strong></td>
      <td align="center">$\mathbf{x}_l = \mathcal{F}(\mathbf{x}) \odot \mathbf{T} + \mathbf{x} \odot (1 - \mathbf{T})$</td>
      <td align="center">需引入额外的门控矩阵 $\mathbf{W}_T$</td>
      <td>
        <del>存在门控阻碍。</del> 梯度必须通过门控系数 $(1 - \mathbf{T})$，并非百分之百自由通透。
      </td>
    </tr>
    <tr bgcolor="#fdfdf0">
      <td><strong>Pre-LN 残差连接 (现代 LLM)</strong></td>
      <td align="center">$\mathbf{x}_l = \mathbf{x}_{l-1} + \mathcal{F}(\operatorname{LN}(\mathbf{x}_{l-1}))$</td>
      <td align="center"><ins><strong>0 额外参数！</strong></ins></td>
      <td>
        <ins><strong>畅通无阻的恒等高速路。</strong></ins> 公式自带 $+\mathbf{I}$ 梯度下限，使 80 层以上的大规模预训练如履平地。
      </td>
    </tr>
  </tbody>
</table>
...
## 步骤 5：手把手超简单数字积木（2 维向量的前向与反向纯手算）
...
<mark>看：原始输入向量 $[2.0, -1.0]^\top$ 构成了信号的骨架，在此基础上仅仅吸收了来自计算层的微小调整 $[-0.0, -0.5]^\top$！</mark>
