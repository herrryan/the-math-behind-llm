...
> 
> You have teddy bears, kittens, toy puppies, wooden airplanes, racecars, fire trucks, bananas, and red apples.
> 
> You want to arrange them on the grass so you can always find what you want quickly:
> - Where do you put the kitten? Right next to the puppy and the teddy bear in the soft, cuddly animal corner!
> - Where do you put the toy airplane? Far away in the "machines that go zoom" corner, right next to the helicopter and rocket ship.
> - Where do you put the red apple? In the tasty snack corner, right next to the banana and orange.
> 
> Now imagine drawing chalk grid lines on the grass:
> - Walk 2 steps North and 8 steps East: you reach the kitten.
> - Walk 9 steps North and 1 step East: you reach the airplane.
> 
> Every toy now has an exact **address** on the grass. 
> 
> If two toys are similar, their addresses are close together. If two toys have nothing in common (like a fluffy kitten and a giant Boeing 747 airplane), their addresses are far apart on opposite sides of the playground.
> 
...
However, computer chips and GPUs are built from silicon and electric circuits. They do not know what the English letters <kbd>"c"</kbd>, <kbd>"a"</kbd>, <kbd>"t"</kbd> mean. A computer processor cannot multiply the word <samp>"kitten"</samp> by $0.90$.
...
$$\mathbf{x}_i = \mathbf{e}_i^\top \mathbf{E} \in \mathbb{R}^{1 \times d}$$
...
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>$\mathbb{R}^{1 \times d}$ (or $\mathbb{R}^d$)</td>
      <td>The specific list of GPS coordinates for word $i$</td>
      <td>$\mathbf{x}_{\text{kitten}} = [0.95, 0.15]$</td>
    </tr>
  </tbody>
</table>
...
Because <samp>"kitten"</samp> and <samp>"cat"</samp> share nearly identical surrounding context words, their mathematical coordinates should be close together.
...
- Distance from <samp>"puppy"</samp> to <samp>"dog"</samp>: $\sqrt{1^2 + (-1)^2} = \sqrt{2} \approx 1.414$
- Distance from <samp>"puppy"</samp> to <samp>"nuclear reactor"</samp>: $\sqrt{1^2 + (-1)^2} = \sqrt{2} \approx 1.414$
...
Subtracting the vector for <samp>"man"</samp> from <samp>"king"</samp> stripped away the "male" concept, leaving behind pure "royalty". Adding <samp>"woman"</samp> yielded the exact coordinates of <samp>"queen"</samp>!
...
The one-hot vector for <samp>"kitten"</samp> (index 2) is:

$$\mathbf{e}_2^\top = \begin{bmatrix} 0 & 1 & 0 & 0 \end{bmatrix}$$

Now, multiply $\mathbf{e}_2^\top$ by $\mathbf{E}$:

$$\mathbf{x}_{\text{kitten}} = \mathbf{e}_2^\top \mathbf{E} = \begin{bmatrix} 0 & 1 & 0 & 0 \end{bmatrix} \begin{bmatrix}
0.90 & 0.30 \\
0.95 & 0.15 \\
0.85 & 0.55 \\
0.02 & 0.98
...
$$\mathbf{x}_{\text{kitten}} = [0.95, 0.15]$$
...
<samp>"airplane"</samp> is nearly **8 times farther** from <samp>"kitten"</samp> than <samp>"cat"</samp> is!
