...
\lt figcaption>\lt strong>Figure 4.2:</strong> Dual-channel forward computation and backward blame attribution within a neuron. The forward pass computes feature representations, while the backward pass backpropagates error signals.</figcaption>
</figure>
...
\lt figcaption>\lt strong>Figure 4.3:</strong> The activation derivative $\sigma'(z)$ acts as a physical coupler or conduit for the backpropagating error signal. If the derivative is zero, the conduit is severed and no learning can occur.</figcaption>
</figure>
...
\lt figcaption>\lt strong>Figure 4.4:</strong> The Vanishing Gradient crisis: For large positive or negative inputs, Sigmoid derivative drops to zero. Multiplying these small derivatives across layers extinguishes training.</figcaption>
</figure>
...
The learning signal diminishes to absolute zero. The earliest layers receive no feedback and cannot update their weights. This catastrophic phenomenon is known as the \lt dfn id="def-vanishing-gradient">\lt strong>Vanishing Gradient Problem</strong></dfn>.

---

### 4. The Second Era: The ReLU Revolution

In 2010&ndash;2012, researchers realized that biological realism was holding back deep learning. They replaced the complicated exponential curve with the simplest conceivable non-linear threshold: the \lt dfn id="def-relu">\lt strong>Rectified Linear Unit (ReLU)</strong></dfn>.
...
\lt table border="1" cellpadding="8" cellspacing="0" width="100%">
  \lt caption>\lt strong>Table 4.1:</strong> Why ReLU Changed Deep Learning Forever</caption>
  \lt thead>
    \lt tr bgcolor="#eae9e1">
      \lt th scope="col" align="left" width="20%">Property</th>
      \lt th scope="col" align="left" width="40%">Sigmoid / Tanh</th>
      \lt th scope="col" align="left" width="40%">ReLU</th>
    </tr>
  </thead>
  \lt tbody>
    \lt tr>
      \lt th scope="row" align="left">\lt strong>Gradient for $z > 0$</strong></th>
      \lt td>Vanishes exponentially ($\to 0$) as $|z|$ grows</td>
      \lt td>\lt strong>Exactly $1.0$</strong> &mdash; gradients flow unhindered across 100+ layers</td>
    </tr>
    \lt tr>
      \lt th scope="row" align="left">\lt strong>Compute Cost</strong></th>
      \lt td>Heavy floating-point exponentials ($e^{-z}$) and division</td>
      \lt td>\lt strong>Single comparison instruction</strong> (\lt code>max(0, x)</code>) on GPU silicon</td>
    </tr>
    \lt tr>
      \lt th scope="row" align="left">\lt strong>Sparsity</strong></th>
      \lt td>Dense: all neurons output non-zero values</td>
      \lt td>\lt strong>True Sparsity</strong>: ~50% of neurons output exactly $0$, saving memory</td>
    </tr>
    \lt tr>
      \lt th scope="row" align="left">\lt strong>Vulnerability</strong></th>
      \lt td>Vanishing gradients halt all deep architectures</td>
      \lt td>\lt dfn id="def-dying-relu">\lt strong>Dying ReLU</strong></dfn>: if a neuron outputs negative, its gradient is 0 forever</td>
    </tr>
  </tbody>
</table>
...
In 2016, Dan Hendrycks and Kevin Gimpel introduced the \lt dfn id="def-gelu">\lt strong>Gaussian Error Linear Unit (GELU)</strong></dfn>.
...
Here, $\Phi(z)$ is the cumulative distribution function (\lt abbr title="Cumulative Distribution Function">CDF</abbr>) of the Gaussian distribution:
...
\lt figcaption>\lt strong>Figure 4.5:</strong> Comparison of ReLU vs. GELU. Notice GELU's smooth curvature: small negative inputs are gently suppressed rather than abruptly extinguished.</figcaption>
</figure>
...
Today, whether you inspect the open-source flagship **LLaMA-3** (Meta), **Mistral** (Europe's AI champion), **Gemma** (Google), **DeepSeek-V2/V3**, or **Qwen-2.5** (Alibaba), their Feed-Forward Networks (\lt abbr title="Feed-Forward Network">FFN</abbr>) have universally discarded traditional ReLU and GELU. In their place stands the undisputed modern standard: \lt dfn id="def-swiglu">\lt strong>SwiGLU</strong></dfn>.
...
In 2017, Yann Dauphin and his team at Meta FAIR engineered this bilinear concept into the \lt dfn id="def-glu">\lt strong>Gated Linear Unit (GLU)</strong></dfn>:
...
To eliminate the Sigmoid ceiling, Google Brain researchers (Ramachandran, Zoph, and Le) used neural architecture search (NAS) to discover \lt dfn id="def-swish">\lt strong>Swish</strong></dfn> (also known as \lt abbr title="Sigmoid Linear Unit">SiLU</abbr>):
...
\lt figcaption>\lt strong>Figure 4.6a:</strong> The Swish activation curve. Unbounded above (shattering Sigmoid's 1.0 ceiling), bounded below (smoothly filtering noise), and equipped with a gentle negative trough (preventing neuron death).</figcaption>
</figure>
...
In a complete Transformer Feed-Forward Network (\lt abbr title="Feed-Forward Network">FFN</abbr>), the input vector is projected up by $\mathbf{W}_{\text{gate}}$ and $\mathbf{W}_{\text{up}}$ simultaneously, gated via element-wise multiplication, and projected back down by $\mathbf{W}_{\text{down}}$:
...
\lt figcaption>\lt strong>Figure 4.6b:</strong> Complete dataflow of the SwiGLU FFN block standard in modern LLMs. Two parallel matrices compute the dynamic gate and the linear candidate value, which modulate each other multiplicatively before down-projection.</figcaption>
</figure>
...
\lt details>
\lt summary>\lt strong>Click to view Formal Mathematical Symbol Catalog</strong></summary>

\lt dl>
  \lt dt>\lt strong>$\mathbf{x} \in \mathbb{R}^{1 \times d_{\text{model}}}$</strong></dt>
  \lt dd>The input activation row vector for a single token at a given layer position.</dd>

  \lt dt>\lt strong>$d_{\text{model}}$</strong></dt>
  \lt dd>The model's internal hidden dimension (e.g., $4096$ in LLaMA-3 8B, $8192$ in LLaMA-3 70B).</dd>

  \lt dt>\lt strong>$d_{\text{ffn}}$</strong></dt>
  \lt dd>The expanded intermediate dimension of the feed-forward network, typically $\approx \frac{8}{3}d_{\text{model}}$ ($14336$ in LLaMA-3 8B).</dd>

  \lt dt>\lt strong>$\mathbf{W}_{\text{gate}} \in \mathbb{R}^{d_{\text{model}} \times d_{\text{ffn}}}$</strong></dt>
  \lt dd>The gate projection matrix that determines the modulation weights.</dd>

  \lt dt>\lt strong>$\mathbf{W}_{\text{up}} \in \mathbb{R}^{d_{\text{model}} \times d_{\text{ffn}}}$</strong></dt>
  \lt dd>The up-projection matrix that produces the candidate information signal.</dd>

  \lt dt>\lt strong>$\mathbf{W}_{\text{down}} \in \mathbb{R}^{d_{\text{ffn}} \times d_{\text{model}}}$</strong></dt>
  \lt dd>The down-projection matrix that returns the gated vector to the model dimension.</dd>

  \lt dt>\lt strong>$\odot$</strong></dt>
  \lt dd>The Hadamard product: component-wise multiplication $(\mathbf{a} \odot \mathbf{b})_i = a_i \cdot b_i$.</dd>

  \lt dt>\lt strong>$\Phi(z)$</strong></dt>
  \lt dd>The Gaussian Cumulative Distribution Function: $\Phi(z) = P(X \le z)$ where $X \sim \mathcal{N}(0, 1)$.</dd>
</dl>
...
\lt dl>
  \lt dt>\lt time datetime="1943">1943</time> &mdash; \lt strong>Warren McCulloch &amp; Walter Pitts</strong>: The Binary Step Neuron</dt>
  \lt dd>
    ...
    \lt br>
    \lt strong>Why it broke:</strong> The derivative of a step function is zero everywhere ($f'(z) = 0$), and undefined at the threshold $\theta$. Gradient descent cannot work if the slope is always zero!
  </dd>

  \lt dt>\lt time datetime="1986">1986</time> &mdash; \lt strong>Rumelhart, Hinton &amp; Williams</strong>: The Smooth Sigmoid Era</dt>
  \lt dd>
    ...
    \lt cite>"Learning representations by back-propagating errors", Nature 1986</cite>.
  </dd>

  \lt dt>\lt time datetime="1991">1991</time> &mdash; \lt strong>Sepp Hochreiter</strong>: The Vanishing Gradient Diagnosis</dt>
  \lt dd>
  ...
  \lt dt>\lt time datetime="2010">2010</time>&ndash;\lt time datetime="2012">2012</time> &mdash; \lt strong>Nair, Hinton &amp; Krizhevsky</strong>: The ReLU Revolution</dt>
  \lt dd>
  ...
  \lt dt>\lt time datetime="2016">2016</time> &mdash; \lt strong>Dan Hendrycks &amp; Kevin Gimpel</strong>: GELU &amp; The Transformer Revolution</dt>
  \lt dd>
    ...
    \lt cite>"Gaussian Error Linear Units (GELUs)", arXiv:1606.08415</cite>.
  </dd>

  \lt dt>\lt time datetime="2020">2020</time> &mdash; \lt strong>Noam Shazeer</strong>: SwiGLU &amp; Modern Gating</dt>
  \lt dd>
    Former Google Brain scientist Noam Shazeer demonstrated that replacing standard FFN activations with multiplicative bilinear gates produces consistent improvements in perplexity and downstream reasoning. Today, almost every leading frontier model &mdash; \lt strong>LLaMA-3</strong> (Meta), \lt strong>Mistral</strong>, \lt strong>Gemma</strong> (Google), \lt strong>DeepSeek</strong>, and \lt strong>Qwen</strong> &mdash; standardizes on SwiGLU.
    \lt cite>"GLU Variants Improve Transformer", arXiv:2002.05202</cite>.
  </dd>
</dl>
...
\lt table border="1" cellpadding="8" cellspacing="0" width="100%">
  \lt caption>\lt strong>Table 4.2:</strong> Hand-Calculated Activation Outputs for $\mathbf{z} = [2.0, 0.0, -1.5]$</caption>
  \lt thead>
    \lt tr bgcolor="#eae9e1">
      \lt th scope="col" align="left">Input $z$</th>
      ...
      \lt th scope="col" align="left">Visual Energy Gauge</th>
    </tr>
  </thead>
  \lt tbody>
    \lt tr>
    ...
    \lt tr>
      ...
      \lt td align="center">$\max(0, -1.5) = \mathbf{0.000}$ \lt del>(hard cutoff)</del></td>
      \lt td align="center">$-1.5 \times \Phi(-1.5) \approx \mathbf{-0.100}$ \lt ins>(soft leak)</ins></td>
      \lt td align="center">$-1.5 \times \sigma(-1.5) \approx \mathbf{-0.274}$ \lt ins>(soft leak)</ins></td>
...
\lt fieldset>
\lt legend>\lt strong>Execution Checklist: Step-by-Step SwiGLU Forward Pass</strong></legend>

\lt p>\lt input type="checkbox" checked disabled> \lt strong>Step 1: Compute Gate Projection</strong>\lt br>
...
\lt p>\lt input type="checkbox" checked disabled> \lt strong>Step 2: Activate the Gate with $\operatorname{Swish}_1$</strong>\lt br>
Modulate gate values smoothly with Swish:</p>
\lt ul>
  \lt li>For $g_1 = -1.0$: $\sigma(-1.0) = \frac{1}{1 + e^1} \approx 0.2689 \implies \operatorname{Swish}(-1.0) = -1.0 \times 0.2689 = \mathbf{-0.269}$</li>
  \lt li>For $g_2 = 2.0$: $\sigma(2.0) = \frac{1}{1 + e^{-2}} \approx 0.8808 \implies \operatorname{Swish}(2.0) = 2.0 \times 0.8808 = \mathbf{1.762}$</li>
</ul>
...
\lt p>\lt input type="checkbox" checked disabled> \lt strong>Step 3: Compute Up Projection (Candidate Signal)</strong>\lt br>
...
\lt p>\lt input type="checkbox" checked disabled> \lt strong>Step 4: Gated Modulation (Hadamard Product $\odot$)</strong>\lt br>
...
\lt p>\lt input type="checkbox" checked disabled> \lt strong>Step 5: Down Projection to Model Dimension</strong>\lt br>
