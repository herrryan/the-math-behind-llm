...
!!! question "计算连接问题: 为什么朴素注意力具有“置换等价性”？"
    ...
    这一数学性质在代数上被称为\lt dfn id="def-permutation-equivariance-zh">置换等价性（Permutation Equivariance）</dfn>。
...
中国学者苏剑林（2021）提出的 \lt dfn id="def-rope-zh">RoPE（Rotary Position Embedding）</dfn> 彻底摒弃了向量加法，转而在 2D 坐标子空间中通过**正交旋转变换**将位置注入 Query 和 Key。
...
\lt details>
\lt summary>\lt strong>数学符号速查与张量定义清单</strong></summary>
\lt dl>
  \lt dt>\lt strong>$m, n \in \mathbb{N}$</strong></dt>
  \lt dd>词元在序列中的绝对时间步坐标：$m$ 为 Query 发起位置，$n$ 为 Key 被检索位置（$0 \le m, n \lt T$）。</dd>
  \lt dt>\lt strong>$d$（头特征维度 Head Dimension）</strong></dt>
  \lt dd>注意力头的特征向量维度（必须为偶数，通常为 $d = 64$ 或 $d = 128$）。</dd>
  \lt dt>\lt strong>$\theta_i$（第 $i$ 组二维平面的基础角频率）</strong></dt>
  \lt dd>空间旋转角速度：$\theta_i = b^{-2(i-1)/d}$，其中底数基频 $b = 10000$（在长文本模型中常提升至 $500000$ 以上）。</dd>
  \lt dt>\lt strong>$\mathbf{R}_{\theta_i, m} \in \mathbb{R}^{2 \times 2}$</strong></dt>
  \lt dd>二维标准正交旋转矩阵：满足 $\mathbf{R}_{\theta_i, m}^\top \mathbf{R}_{\theta_i, m} = \mathbf{I}_2$，行列式 $\det(\mathbf{R}) = 1$。</dd>
  \lt dt>\lt strong>$\mathbf{R}_{\Theta, m}^d \in \mathbb{R}^{d \times d}$</strong></dt>
  \lt dd>由 $\frac{d}{2}$ 个二维旋转块构成的分块对角正交旋转矩阵。</dd>
  \lt dt>\lt strong>$\widetilde{\mathbf{q}}_m, \widetilde{\mathbf{k}}_n \in \mathbb{R}^d$</strong></dt>
  \lt dd>完成位置角度旋转后的位置敏感型 Query 与 Key 向量。</dd>
</dl>
</details>
...
\lt figcaption>\lt strong>图 10.2：</strong> 从无序词袋到乘性旋转几何的代际技术演化。</figcaption>
</figure>
...
\lt fieldset>
\lt legend>\lt strong>四代位置表征方案的优势与致命软肋</strong></legend>

\lt table border="1" cellpadding="8" cellspacing="0" width="100%">
  \lt caption>\lt strong>表 10.1：</strong> 语言模型主流位置编码机制横向对比。</caption>
  \lt thead>
    \lt tr bgcolor="#f0eee6">
      \lt th align="left">编码方案</th>
      \lt th align="center">数学公式</th>
      \lt th align="left">代表模型</th>
      \lt th align="left">核心缺陷与技术瓶颈</th>
    </tr>
  </thead>
  \lt tbody>
    \lt tr>
      \lt td>\lt strong>正弦绝对编码</strong></td>
      \lt td align="center">$\mathbf{x}_m + \mathbf{p}_m$</td>
      \lt td>Transformer (2017)</td>
      \lt td>
        \lt del>加性噪音污染！</del> 强行将坐标加进语义嵌入。展开点积时产生非预期的内容-位置交叉耦合项。
      </td>
    </tr>
    \lt tr>
      \lt td>\lt strong>可学习绝对嵌入</strong></td>
      \lt td align="center">$\mathbf{x}_m + \mathbf{W}_{\text{pos}}[m]$</td>
      \lt td>BERT (2018), GPT-2 (2019)</td>
      \lt td>
        \lt del>硬编码长度天花板！</del> 若查找表最大为 $T_{\max} = 2048$，则第 $2049$ 步无参数可用，模型完全无法外推更长文本。
      </td>
    </tr>
    \lt tr>
      \lt td>\lt strong>相对偏置（T5 / Shaw）</strong></td>
      \lt td align="center">$S_{ij} + b_{i-j}$</td>
      \lt td>Shaw (2018), T5 (2020)</td>
      \lt td>
        \lt del>显存带宽黑洞！</del> 必须在注意力打分上叠加一个 $T \times T$ 的全局偏置张量，与片上高速融合算子（FlashAttention）水火不容。
      </td>
    </tr>
    \lt tr bgcolor="#fdfdf0">
      \lt td>\lt strong>旋转位置嵌入（RoPE）</strong></td>
      \lt td align="center">$\mathbf{R}_m \mathbf{q}_m, \, \mathbf{R}_n \mathbf{k}_n$</td>
      \lt td>LLaMA, Mistral, Qwen, DeepSeek</td>
      \lt td>
        \lt ins>\lt strong>现代 AI 的黄金标准！</strong></ins> 相对距离严格等价于旋转差 $\mathbf{R}_{n-m}$，零显存查找表，不干扰 Value，天然支持 128k 超长文本外推！
      </td>
    </tr>
  </tbody>
</table>
...
\lt p align="center">
  \lt kbd>词元 A: Query 发起者，位于位置 m = 1</kbd> &emsp;
  \lt kbd>词元 B: Key 响应者，位于位置 n = 3</kbd>
</p>
...
\lt mark>两路手算结果在小数点后第 4 位丝毫不差、完全吻合！</mark>
