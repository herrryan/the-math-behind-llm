...
## Step 1: 3-Year-Old Intuition (The Giant Playground Map) {: #step-1 }

!!! note "3-Year-Old Intuition: 3-Year-Old Intuition: The Giant Playground Map"
...
## Step 2: The Bridging Question (From Words to Numbers) {: #step-2 }
...
\lt figcaption>\lt strong>Figure 1.1:</strong> Vector lookup formulated as one-hot matrix multiplication: the solitary 1 selects row i.</figcaption>
</figure>

!!! note "Note: Practical Engineering Note: Lookup vs. Matrix Multiply"
...
\lt table border="1" cellpadding="8" cellspacing="0" width="100%">
  \lt caption>\lt strong>Table 1.1:</strong> Mathematical symbols, dimensions, and physical meanings for vector embeddings.</caption>
  \lt thead>
    \lt tr>
      \lt th align="left">Symbol</th>
      \lt th align="left">Formal Name</th>
      \lt th align="left">Shape / Dimension</th>
      \lt th align="left">3-Year-Old Meaning</th>
      \lt th align="left">Concrete Example</th>
    </tr>
  </thead>
  \lt tbody>
    \lt tr>
      ...
      \lt td>Set of tokens</td>
      \lt td>The master toy box of all known words</td>
      \lt td>$V = \{\text{"cat"}, \text{"kitten"}, \text{"dog"}, \text{"airplane"}\}$</td>
    </tr>
    \lt tr>
      \lt td>$|V|$</td>
      \lt td>Vocabulary Size</td>
      \lt td>Scalar integer</td>
      \lt td>Total count of words in the toy box</td>
      \lt td>$|V| = 4$ in toy model ($32{,}000$ to $128{,}000$ in LLMs)</td>
    </tr>
    \lt tr>
      \lt td>$d$</td>
      \lt td>Embedding Dimension</td>
      \lt td>Scalar integer</td>
      \lt td>How many chalk directions are on our playground map</td>
      \lt td>$d = 2$ in our toy example ($4{,}096$ in LLaMA-3-8B)</td>
    </tr>
    \lt tr>
      \lt td>$\mathbf{e}_i$</td>
      \lt td>One-Hot Vector</td>
      \lt td>$\mathbb{R}^{|V| \times 1}$</td>
      \lt td>A light switch board with only switch $i$ turned ON</td>
      \lt td>$\mathbf{e}_2 = [0, 1, 0, 0]^\top$ (selects word 2)</td>
    </tr>
    \lt tr>
      \lt td>$\mathbf{E}$</td>
      \lt td>Embedding Matrix</td>
      \lt td>$\mathbb{R}^{|V| \times d}$</td>
      \lt td>The master address book storing coordinates for every toy</td>
      \lt td>Matrix of shape $4 \times 2$</td>
    </tr>
    \lt tr>
      \lt td>$\mathbf{x}_i^\top$ (or $\mathbf{x}_i$)</td>
      \lt td>Embedding Vector</td>
      \lt td>$\mathbb{R}^{1 \times d}$ (row) / $\mathbb{R}^{d \times 1}$ (column)</td>
      \lt td>The specific list of GPS coordinates for word $i$</td>
...
\lt dl>
  \lt dt>\lt strong>Dense Vector</strong></dt>
  \lt dd>A vector where almost all entries are non-zero real numbers ($\mathbb{R}$), packing rich semantic information into a compact dimensional space.</dd>
  
  \lt dt>\lt strong>Sparse Vector</strong></dt>
  \lt dd>A vector where the overwhelming majority of entries are zero (such as a one-hot vector with a single 1 and 99,999 zeros).</dd>
  
  \lt dt>\lt strong>Embedding Space ($\mathbb{R}^d$)</strong></dt>
  \lt dd>The $d$-dimensional continuous vector space spanned by the coordinate axes where semantic concepts live as geometric points.</dd>
  
  \lt dt>\lt strong>Kronecker Delta ($\delta_{ij}$)</strong></dt>
  \lt dd>A binary piecewise function that equals 1 if indices $i$ and $j$ match, and 0 otherwise.</dd>
</dl>
</details>
...
!!! quote "Historical Origin & Derivation: The Historical Journey: From Firth to Word2Vec"
...
- \lt kbd>"The fluffy [kitten] drank warm milk from the saucer."</kbd>
- \lt kbd>"The fluffy [cat] drank warm milk from the saucer."</kbd>

Because \lt kbd>"kitten"</kbd> and \lt kbd>"cat"</kbd> share nearly identical surrounding context words, their mathematical coordinates should be close together.
...
- Distance from \lt kbd>"puppy"</kbd> to \lt kbd>"dog"</kbd>: $\sqrt{1^2 + (-1)^2} = \sqrt{2} \approx 1.414$
- Distance from \lt kbd>"puppy"</kbd> to \lt kbd>"nuclear reactor"</kbd>: $\sqrt{1^2 + (-1)^2} = \sqrt{2} \approx 1.414$
...
Subtracting the vector for \lt kbd>"man"</kbd> from \lt kbd>"king"</kbd> stripped away the "male" concept, leaving behind pure "royalty". Adding \lt kbd>"woman"</kbd> yielded the exact coordinates of \lt kbd>"queen"</kbd>!
...
!!! tip "Toy Example Walkthrough: Concrete Toy Example: The 2D Pet-and-Vehicle Playground"
...
The one-hot vector for \lt kbd>"kitten"</kbd> (index 2) is:
