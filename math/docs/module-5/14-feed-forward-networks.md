...
\lt figcaption>\lt strong>Figure 14.2:</strong> SwiGLU splits the expansion into a continuous gate branch and a value branch, enabling multiplicative feature filtering before down-projection.</figcaption>
</figure>
...
\lt figcaption>\lt strong>Figure 14.3:</strong> The first matrix acts as a collection of memory keys that detect input patterns; the second matrix acts as memory values that inject factual knowledge.</figcaption>
</figure>
...
\lt p>\lt input type="checkbox" checked disabled> \lt strong>Step A:</strong> Project into expansion space $\mathbf{z} = \mathbf{x}\mathbf{W}_1$.</p>
\lt p>\lt input type="checkbox" checked disabled> \lt strong>Step B:</strong> Apply non-linear activation $\mathbf{a} = \operatorname{ReLU}(\mathbf{z})$.</p>
\lt p>\lt input type="checkbox" checked disabled> \lt strong>Step C:</strong> Project back to model dimension $\mathbf{y} = \mathbf{a}\mathbf{W}_2$.</p>
</fieldset>
...
\lt mark>Notice the sparsity: Keys 3 and 4 were zeroed out completely!</mark>
