...
人工神经网络究竟是如何同时移动、旋转、扭转成千上万个词向量的？

我们该如何把整场拉伸橡胶布的物理几何游戏，浓缩进一张工整的数字表格中，让计算机芯片在几微秒之内完成极速计算？

---

<h2 id="step-3">Step 3: 精确数学公式</h2>

在现代大语言模型的每一个层级中，重塑向量空间都依托于深度学习最根本的基础方程式：**仿射线性变换（Affine Linear Transformation）**。

---

### 1. 核心变换方程式

对于一个维度为 $k$ 的输入向量 $\mathbf{x} \in \mathbb{R}^{k \times 1}$、一个权重矩阵 $\mathbf{W} \in \mathbb{R}^{m \times k}$，以及一个偏置向量 $\mathbf{b} \in \mathbb{R}^{m \times 1}$，变换后的输出向量 $\mathbf{y} \in \mathbb{R}^{m \times 1}$ 为：
...
在这个公式中：
- $\mathbf{W}\mathbf{x}$ 负责执行**橡胶布拉伸**：拉伸、旋转、倾斜剪切，或者改变空间的维度大小。
...
### 3. 几何秘密：基向量最终落在了何处？（大模型视角的深度动机）

为什么研究大语言模型的研究人员要如此执着于“基向量落在了何处”？
...
虽然这解释了芯片在底层如何进行微观计算，但它把整个神经网络层变成了一张毫无生机的冰冷数字网格，无法帮我们建立对大模型特征处理的直观物理认知。

从**矩阵的列（基向量的归宿）**来审视矩阵乘法，才能揭示大语言模型重塑语义特征的真正工程蓝图。

#### 大模型里的“基向量”究竟是什么？

在标准的 2 维特征空间中，任意一个词元向量 $\mathbf{x} = \begin{bmatrix} x_1 \\ x_2 \end{bmatrix}$ 都是由两个基础单位箭头——**标准基向量**组合而成的：
- $\hat{\mathbf{i}} = \begin{bmatrix} 1 \\ 0 \end{bmatrix}$ 代表一个拥有 **100% 纯粹第1特征**（例如：纯粹的“猫科属性”）、其余属性完全为 0 的极端概念。
- $\hat{\mathbf{j}} = \begin{bmatrix} 0 \\ 1 \end{bmatrix}$ 代表一个拥有 **100% 纯粹第2特征**（例如：纯粹的“活泼程度”）、其余属性完全为 0 的极端概念。

语言空间里的每一个具体单词，都只是这两样纯粹特征原料的调配产物：$\mathbf{x} = x_1 \hat{\mathbf{i}} + x_2 \hat{\mathbf{j}}$。

#### 矩阵各列：神经网络的“特征转译词典”

现在，请观察当权重矩阵 $\mathbf{W} = \begin{bmatrix} W_{1,1} & W_{1,2} \\ W_{2,1} & W_{2,2} \end{bmatrix}$ 作用在这些纯粹基向量特征上时，会发生什么：
...
> **权重矩阵的每一列，正是该层将纯粹基准特征变换后，它们在输出空间中着陆的新坐标！**

矩阵 $\mathbf{W}$ 的每一列，实际上是该网络层编写的一条**特征转译词典词条**：
- **第 1 列（$\mathbf{w}_{:, 1}$）**回答了：*“如果输入词元携带 1 个单位的第1特征（猫科属性），本层应该为它催生出怎样的新下游特征（例如：捕食习性、家宠亲和力）？”*
- **第 2 列（$\mathbf{w}_{:, 2}$）**回答了：*“如果输入词元携带 1 个单位的第2特征（活泼度），本层应该为它催生出怎样的新下游特征？”*

#### 矩阵乘法本质是“概念配方搅拌机”

当一个真实的词向量 $\mathbf{x} = \begin{bmatrix} x_1 \\ x_2 \end{bmatrix}$（如 $\text{"cat"} = [2, 1]^\top$）进入神经网络层时，矩阵乘法在几何上执行的是：

$$
\mathbf{W}\mathbf{x} = x_1 (\mathbf{W} \text{ 的第 1 列}) + x_2 (\mathbf{W} \text{ 的第 2 列})
$$

输入向量 $\mathbf{x}$ 绝非冰冷的数学坐标，它是一份**概念搅拌配方**：
> *“请从第 1 列的概念中取 $x_1$ 份，从第 2 列的概念中取 $x_2$ 份，将它们充分混合，作为输出表征！”*

<fieldset>
<legend><strong>为什么“基向量-列视角”能彻底贯通后续 Transformer 架构？</strong></legend>
掌握矩阵的列视角，是理解后续 Transformer 核心机制的关键钥匙：
<ol>
  <li><strong>自注意力投影矩阵（$\mathbf{W}_Q, \mathbf{W}_K, \mathbf{W}_V$）（第3模块）</strong>：当词元乘以 $\mathbf{W}_Q$ 时，$\mathbf{W}_Q$ 的各列定义了“查询空间”的全新坐标轴——将原始词汇特征转译为<em>“当前词元正在向上下文探寻什么问题？”</em>。</li>
  <li><strong>前馈记忆网络（FFN / MLP）（第5模块）</strong>：在 LLaMA 和 GPT 中，中间层将向量升维至上万维再降维投影。这些矩阵的列构成了大模型的键值记忆槽，存储着庞大的事实性知识联想。</li>
  <li><strong>机制可解释性（Mechanistic Interpretability）</strong>：当 AI 安全研究人员尝试在大模型内部寻找“拒绝回答方向”、“诚实向量”或“幻觉回路”时，他们分析的正是这些矩阵各列所张成的线性组合方向。</li>
</ol>
</fieldset>
...
- 输入矩阵为 $\mathbf{X} \in \mathbb{R}^{T \times d_{\text{in}}}$（每一行是一个词元）。
...
<table border="1" cellpadding="8" cellspacing="0" width="100%">
  <caption><strong>表 3.1:</strong> 第03章矩阵运算的形式化数学符号、维度与物理含义</caption>
  <thead>
    <tr bgcolor="#f0f0f0">
