...
在标准的二维平面直角坐标系中，任意一个点 $\mathbf{x} = \begin{bmatrix} x_1 \\ x_2 \end{bmatrix}$ 都是由两个最基础的单位箭头——**标准基向量**搭建而成的：
- 水平基向量：$\hat{\mathbf{i}} = \begin{bmatrix} 1 \\ 0 \end{bmatrix}$
- 垂直基向量：$\hat{\mathbf{j}} = \begin{bmatrix} 0 \\ 1 \end{bmatrix}$

任何向量都只是这两个基底的步数合成：$\mathbf{x} = x_1 \hat{\mathbf{i}} + x_2 \hat{\mathbf{j}}$。

现在，让我们看看权重矩阵 $\mathbf{W} = \begin{bmatrix} W_{1,1} & W_{1,2} \\ W_{2,1} & W_{2,2} \end{bmatrix}$ 作用在基向量上时会发生什么：
...
这就是线性代数中最震撼人心的终极几何真谛：

> **矩阵的每一列，仅仅是拉伸橡胶布之后，原本各个单位基向量所着陆的新坐标！**

只要你掌握了 $\hat{\mathbf{i}}$ 降落到了何处（第 1 列），以及 $\hat{\mathbf{j}}$ 降落到了何处（第 2 列），你就瞬间知晓了*整个宇宙中任何一个向量*拉伸后的归宿，因为线性组合被完整保留：

$$
\mathbf{W}\mathbf{x} = x_1 (\text{第 1 列}) + x_2 (\text{第 2 列})
$$
