...
**微积分反向传播的降维打击**：
借助微积分链式法则，反向传播算法仅需在模型前向运行后进行一次反向遍历（耗时仅约前向的 2 倍），就能在数毫秒内**同时、精确地解析出全部 700 亿个参数对损失的敏感程度**！

#### 链式法则微观解剖
考虑一个单神经元结构，它计算加权线性组合 $z = \sum_k w_k x_k + b$，随后经过激活函数 $a = \sigma(z)$。根据微积分链式法则，损失 $\mathcal{L}$ 对权重 $w_k$ 的偏导数（敏感度）可严格展开为：
